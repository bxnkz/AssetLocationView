...
const Navbar: React.FC<NavBarProps> = ({
  ...
  const availableDepartments = floorToDeptMap[selectedFloor] || [];
  useEffect(() => {
    if (
      selectedDepartment &&
  ...
  return (
    <nav className="flex flex-wrap md:flex-nowrap justify-between items-center bg-blue-900 text-white px-5 py-2 shadow-md">
      ...
      <div className="w-full md:w-auto order-3 md:order-2 mt-4 md:mt-0">
        <ul className="flex flex-col md:flex-row gap-4 md:gap-5 m-0 p-0 list-none">
          <label  className="flex flex-col md:flex-row md:items-center" htmlFor="">
            Select Site :
            <select name="" id="" value={selectedSite} onChange={(e) => onSiteChange(e.target.value)} className="md:ml-2 rounded-full bg-gray-400 border-black text-black">
              <option className="text-center" value="B4">B4</option>
              {/* <option value="Fl2">CD</option> */}
            </select>
          </label>        
          <label className="flex flex-col md:flex-row md:items-center" htmlFor="">
            Select Floor :
            <select name="" id="" value={selectedFloor} onChange={(e) => onFloorChange(e.target.value)} className="md:ml-2 rounded-full  bg-gray-400 border-black text-black">
            ...
            <label className="flex flex-col md:flex-row md:items-center" htmlFor="">
              Select Department :
              <select name="" id="" value={selectedDepartment} onChange={(e) => onDepartmentChange(e.target.value)} className="md:ml-2 rounded-full bg-gray-400 border-black text-black">
                <option className="text-center" value="">-- Select Dept --</option>

              {availableDepartments.map((dept) => (
                  <option className="text-center"key={dept} value={dept}>{dept}</option>
                ))}
              </select>
            </label>
        </ul>
